...
def upload():
    ...
    try:
        ...
        flash(f'Successfully uploaded {len(transactions)} transactions.', 'success')
...
def monthly_summary():
    ...
    for row in results:
        ...
        if key not in summary:
            summary[key] = {'year': int(row.year), 'month': int(row.month), 'credit': 0, 'debit': 0, 'credit_count': 0, 'debit_count': 0}
